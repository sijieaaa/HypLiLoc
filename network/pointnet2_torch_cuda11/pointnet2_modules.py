...
class _PointnetSAModuleBase(nn.Module):
    ...
    def forward(self, xyz: torch.Tensor,
        ...
        new_xyz = new_xyz.transpose(1, 2).contiguous() if self.npoint is not None else None
...
class PointnetLFPModuleMSG(nn.Module):
    ...
    def forward(self, xyz2: torch.Tensor, xyz1: torch.Tensor,
                features2: torch.Tensor, features1: torch.Tensor):
...
if __name__ == "__main__":
    ...
    xyz_feats = torch.randn(2, num_points, 6).cuda()
    ...
    test_module = PointnetSAModuleMSG(
        ...
        radii=[5.0], 
        nsamples=[12], 
        mlps=[[num_points, 20]] # out_channels
    )



    test_module.cuda()
    xyz, point_features = test_module(xyz, xyz_feats)
    # [b,500,3]  [b,20,500]
